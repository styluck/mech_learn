...
class Residual3(tf.keras.Model):
    def __init__(self, num_channels1, num_channels2, use_1x1conv=False, strides=1):
        super().__init__()
        self.conv1 = layers.Conv2D(
            num_channels1, padding='same', kernel_size=3, strides=strides)
        self.bn1 = layers.BatchNormalization()
        
        self.conv2 = layers.Conv2D(
            num_channels1, kernel_size=3, padding='same')
        self.bn2 = layers.BatchNormalization()
        
        self.conv3 = layers.Conv2D(
            num_channels2, kernel_size=1, padding='same')
        self.bn1 = layers.BatchNormalization()
        
        self.conv4 = None
        if use_1x1conv:
            self.conv4 = layers.Conv2D(
                num_channels2, kernel_size=1, strides=strides)

    def call(self, X):
        Y1 = tf.keras.activations.relu(self.bn1(self.conv1(X)))
        Y2 = tf.keras.activations.relu(self.bn2(self.conv2(Y1)))
        Y = self.bn3(self.conv3(Y2))
